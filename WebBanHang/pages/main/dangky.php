<div class="main_content">
    ...
    if (isset($_POST['dang_ky'])) {
        ...
        } elseif (!preg_match("/[A-Z]/", $mat_khau)) {
            $registration_error = "Mật khẩu phải có ít nhất một chữ viết hoa!";
        } elseif (!preg_match("/[a-z]/", $mat_khau)) {
            $registration_error = "Mật khẩu phải có ít nhất một chữ viết thường!";
        } elseif (!preg_match("/[0-9]/", $mat_khau)) {
            $registration_error = "Mật khẩu phải có ít nhất một chữ số!";
...
<script>
    ...
    var phonePattern = /^0[0-9]{9,10}$/; // Kiểm tra số điện thoại bắt đầu bằng số 0
    var isValid = true;
    ...
    if (dien_thoai.trim() === '') {
        ...
        isValid = false;
    } else if (!/^0/.test(dien_thoai)) {
        // Số điện thoại không bắt đầu bằng số 0
        dien_thoai_error.textContent = 'Số điện thoại phải bắt đầu bằng số 0';
        dien_thoai_error.style.display = 'flex';
        isValid = false;
    } else if (dien_thoai.length < 10 || dien_thoai.length > 11) {
        // Số điện thoại có độ dài không hợp lệ (phải từ 10 đến 11 chữ số)
        dien_thoai_error.textContent = 'Số điện thoại phải có từ 10 đến 11 chữ số';
        dien_thoai_error.style.display = 'flex';
        isValid = false;
    } else {
