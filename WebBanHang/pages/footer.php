<div class="clear"></div> <!-- gi day ?? -->
<div class="footer">
    <div class="footer_content">
        <div class="footer_info">
            <p class="title">Thông tin chung</p>
            <a class="info">Trang chủ</a>
            <a class="info">Liên hệ</a>
            <a class="info">Tin tức</a>
        </div>
        <div class="footer_info">
            <p class="title">Danh mục</p>
            <a class="info">Áo đấu</a>
            <a class="info">Áo khoác</a>
            <a class="info">Áo Retro</a>
            <a class="info">Phụ kiện</a>
        </div>
        <div class="footer_info">
            <p class="title">Thông tin chung</p>
            <a href="index.php?quanly=CSQDC" class="info">Chính sách và quy định chung</a>
            <a href="index.php?quanly=QDHTTT" class="info">Quy định và hình thức thanh toán</a>
            <a href="index.php?quanly=CSVC" class="info">Chính sách vận chuyển, giao nhận</a>
            <a href="index.php?quanly=CSBH" class="info">Chính sách bảo hành</a>
            <a href="index.php?quanly=CSDT" class="info">Chính sách đổi trả sản phẩm</a>
            <a href="index.php?quanly=CSBM" class="info">Chính sách bảo mật</a>
        </div>
        <div class="footer_info">
            <p class="title">7TCC Blogs</p>
            <a class="info">Trang chủ</a>
            <a class="info">Liên hệ</a>
            <a class="info">Tin tức</a>
        </div>
    </div>
    <div class="footer_content">
        <div class="footer_info">
            <p class="title">Phương thức thanh toán</p>
            <div class="sda">
                <img class="thanhtoan_img" src="/WebBanHang/images/atm.png" alt="">
                <img class="thanhtoan_img" src="/WebBanHang/images/jcb.png" alt="">
                <img class="thanhtoan_img" src="/WebBanHang/images/debit.png" alt="">
                <img class="thanhtoan_img" src="/WebBanHang/images/visa.png" alt="">
            </div>
        </div>
        <div class="footer_info">
            <p class="title">Kết nối với chúng tôi</p>
            <div class="sda" style="display : flex; flex-direction : row; gap : 10px">
                <a href="#">
                    <div class="contact_icon" style="background-color: rgba(29, 161, 242, 1);">
                        <img style="width: 18px;height: 18px; " src="/WebBanHang/images/twitter.svg" alt="">
                    </div>
                </a>
                <a href="#">
                    <div class="contact_icon" style="background-color: rgba(59, 89, 152, 1);">
                        <img style="width: 18px;height: 18px; " src="/WebBanHang/images/facebook.svg" alt="">
                    </div>
                </a>
                <a href="#">
                    <div class="contact_icon" style="background-color: rgba(221, 75, 57, 1);">
                        <img style="width: 18px;height: 18px; " src="/WebBanHang/images/gmail.svg" alt="">
                    </div>
                </a>
                <a href="#">
                    <div class="contact_icon" style="background-color: rgba(30, 136, 229, 1);">
                        <img style="width: 18px;height: 18px; " src="/WebBanHang/images/instagram.svg" alt="">
                    </div>
                </a>
                <a href="#">
                    <div class="contact_icon" style="background-color: rgba(225, 0, 0, 1);">
                        <img style="width: 18px;height: 18px; " src="/WebBanHang/images/youtube.svg" alt="">
                    </div>
                </a>
            </div>
        </div>
        <iframe style="width : 450px; height : 230px;" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d125380.16760889493!2d106.52379583426023!3d10.877697566303755!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x31752a742dabfd33%3A0xd99a9f06eac63692!2zSMOzYyBNw7RuLCBUaMOgbmggcGjhu5EgSOG7kyBDaMOtIE1pbmgsIFZp4buHdCBOYW0!5e0!3m2!1svi!2s!4v1680348941697!5m2!1svi!2s" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
    </div>
    <div class="footer_copyright">Copy right by 7TCC| For True Fans Since 2024.</div>
</div>